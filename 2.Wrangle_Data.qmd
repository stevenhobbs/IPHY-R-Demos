...
The level names of factor variables can be changed with the `factor`, `recode`, or `fct_recode` commands. These commands produce the same result, but structure the key arguments differently. The `factor` command uses a labels argument that lists new level names in the numeric or alphabetic order of existing names.

`variable = factor(variable, labels = c("new name 1", "new name 2"))`

The fct_recode and recode commands use an argument that equates new to existing level names, but order terms around the equality differently.

`variable = recode(variable, "0" = "new name 1, "1" = "new name 2")`

`variable = fct_recode(variable, "new name 1" = "0", "new name 2" = "1")`

Because existing names need not be referenced with the `factor` command, this command is often shorter than the other two. However, the `recode` and `fct_recode` commands are more easily used to change a subset of level names, or even a single level name. Below we use each of these commands to recode a factor variable in our cholesterol data:
